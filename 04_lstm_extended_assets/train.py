...
# ── Config ─────────────────────────────────────────────────────────────────────
DATA_PATH  = "data/training_data_extended_clean.csv"
...
TRAIN_END = pd.Timestamp("2025-03-25")
VAL_END   = pd.Timestamp("2026-03-25")
...
def train_model(model, train_loader, val_loader,
    ...
    for epoch in range(max_epochs):
        ...
        # ── Step 1: Train (don't collect returns) ──
        model.train()
        ...
        # ── Step 2: Clean training evaluation with final θ ──
        model.eval()
        all_train_rets = []
        with torch.no_grad():
            for X_batch, Y_batch in train_loader:
        ...
        # Validation (already in eval mode)
        model.eval()
        all_val_rets = []
        with torch.no_grad():
